...
class discord_bot(discord.Client):
    ...
    def get_guild_temp_data(self, guild_id):
        with self.global_lock:
    ...
    def update_guild_config(self, guild_id):
    ...
    def update_channel_data(self, channel):
        channel_data = self.get_channel_data(channel.guild.id, channel.id)
    ...
    async def temporary_message(self, channel, content = None, embed = None):
        ...
        with lock:
            temp_channel_data = self.get_channel_temp_data(channel.guild.id, channel.id)
            channel_data = self.get_channel_data(channel.guild.id, channel.id)
    ...
    async def on_ready(self):
        with self.global_lock:
            ...
            for guild_id_str in os.listdir(guilds_path):
                ...
                self.guilds_data[guild_id]['config'] = self.try_load_json(os.path.join(guild_directory, "config.json"), self.default_guild_config())
                self.get_guild_data(guild_id)
                self.guilds_data[guild_id]['data'] = self.try_load_pickle(os.path.join(guild_directory, "data.p"), self.default_guild_data())
                ...
                for channel_id_str in os.listdir(os.path.join(guild_directory, "channels")):
                    ...
                    self.get_channel_config(guild_id, channel_id)
                    ...
                    self.get_channel_data(guild_id, channel_id)
